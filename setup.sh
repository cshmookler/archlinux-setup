...
export SETUP_BASE_PACKAGES="base base-devel linux linux-firmware networkmanager bash bash-completion man-db man-pages texinfo curl git zip unzip python htop lynx ufw transmission-cli openssh openssl openvpn borg python-llfuse bluez bluez-utils dictd cmake neovim"
